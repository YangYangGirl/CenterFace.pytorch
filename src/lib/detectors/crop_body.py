...
from utils.post_process import crop_body_post_process

from .base_detector import BaseDetector

class CropBodyDetector(BaseDetector):
  def __init__(self, opt):
    super(CropBodyDetector, self).__init__(opt)
  ...
  def process(self, images, return_time=False):
    with torch.no_grad():
      ...
      # import pdb; pdb.set_trace()
      # cv2.imwrite('0203_pred_output_hm_hp_mse_21_mobilev3_10.jpg', hm_vis)
      cv2.imwrite('../output/imgs/' + 'hm_hp_pred_{}'.format(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]) + ".png", hm_vis)
      
      # import pdb; pdb.set_trace()  
      hm_vis = 255 - sum(output['hps'][0].cpu().clone().detach()) * 255
      hm_vis = np.clip(hm_vis, 0, 255)
      hm_vis = np.array(hm_vis, dtype=np.uint8)
      hm_vis = np.expand_dims(hm_vis, axis=-1)
      hm_vis = np.repeat(hm_vis, 3, axis=-1)
      # import pdb; pdb.set_trace()
      # cv2.imwrite('0203_pred_output_hm_hp_mse_21_mobilev3_10.jpg', hm_vis)
      cv2.imwrite('../output/imgs/' + 'hps_pred_{}'.format(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]) + ".png", hm_vis)
      ...
      dets = multi_pose_decode(
        ...
        reg=reg, hm_hp=hm_hp, hp_offset=hp_offset, K=40)
  ...
  def show_results(self, debugger, image, results):
    debugger.add_img(image, img_id='multi_pose_crop')
    for bbox in results[1]:
      if bbox[4] > 0.01:#self.opt.vis_thresh:
        # debugger.add_coco_bbox(bbox[:4], 0, bbox[4], img_id='multi_pose')
        debugger.add_whole_body_points(bbox[5:47], img_id='multi_pose_crop')
        ...
        cv2.imwrite('../output/imgs/' + 'debug_pred_{}'.format(datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]) + ".png", debug_image)
  ...
  def return_results(self, debugger, image, results):
    ...
    for bbox in results[1]:
      if bbox[4] > 0.2:#self.opt.vis_thresh:
        # debugger.add_coco_bbox(bbox[:4], 0, bbox[4], img_id='multi_pose')
        debugger.add_whole_body_points(bbox[5:47], img_id='multi_pose_crop')
    return debugger.return_img(img_id='multi_pose_crop')