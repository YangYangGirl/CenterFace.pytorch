...
def whole_body_decode(
  ...
  if hm_hp is not None:
    #   hm_hp = _nms(hm_hp)                                         # 第二次：通过关节点热力图求得关节点的中心点
      ...
      print("k"*10, K)
      hm_score, hm_inds, hm_ys, hm_xs = _topk_channel(hm_hp, K=21) # b x J x K
...
#     if hm_hp is not None:
#         hm_vis = 255 - sum(hm_hp[0].cpu()) * 255
...
#         cv2.imwrite('pred_hm_hp_new.jpg', hm_vis)
...
#         hm_vis = np.expand_dims(hm_vis, axis=-1)
#         hm_vis = np.repeat(hm_vis, 3, axis=-1)
#         cv2.imwrite('_nms_hm_hp_new.jpg', hm_vis)
