...
pretrained_models_path = '/home/yangy/CenterFace.pytorch/pretrained/vovnet'
...
vovnet_configs = {
    ...
    'VoVNet19_slim_se': {
        ...
        'per_stage_inter_channels': [64, 80, 96, 112],
        'per_stage_inout_channels': [112, 256, 384, 512],  #yy change 512 to 64
        'per_block_layer_nums': 3,
        'per_stage_block_nums': [1, 1, 1, 1],
        "has_se": True,
        'has_depthwise': False,
...
class VoVNet(nn.Module):
    def __init__(self, heads, head_conv, width_mult=1.0, vovnet_type='VoVNet19_slim_se', num_classes=1000):
...
def _vovnet(heads='', head_conv='', width_mult=1.0, arch="VoVNet19_slim_se", pretrained=False, progress=True, **kwargs):
    model = VoVNet(heads, head_conv, width_mult, arch, **kwargs)
...
def get_vovnet(num_layers, heads, head_conv=24, **kwargs):
  model = _vovnet(heads, head_conv, width_mult=1.0, arch='VoVNet19_slim_se', pretrained=False, progress=True, **kwargs)
