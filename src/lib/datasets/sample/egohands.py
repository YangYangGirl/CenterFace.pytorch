...
class EgohandsDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    file_name = self.images_name[index]
    anns = self.anns[file_name]
    ...
    cv2.imwrite("origin_egohands_100_out.jpg", result_img)
    ...
    for k in range(num_objs):
      ann = anns[k]
      bbox = np.array(ann[-4:], dtype=np.float32)
      cls_id = 0
    ...
    # img = cv2.resize(img, (int(input_w/4), int(input_h/4))) 
    masked_image =  hm_vis * 0.9 + img * 0.1
    cv2.imwrite('debug_egohands_hm_hp.jpg', masked_image)
