...
class CropBodyDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    cv2.imwrite("origin_body_100_out_new.jpg", result_img)
    ...
    #trans_input =  np.array([[4 * self.opt.input_res / width, 4 * self.opt.input_res / height, 1], [4 * self.opt.input_res / width, 4 * self.opt.input_res / height, 1]])
    ...
    trans_output = np.array([output_res / width, output_res / height, 1])#, [output_res / width, output_res / height, 1]])
    trans_output_rot = trans_output
    ...
    cv2.imwrite('debug_whole_body_hm_hp_100_new.jpg', masked_image)
    ...
    for m in gt_det:
      each_gt_det = m[:4]
      each_gt_det = np.array(each_gt_det, dtype=np.int32)
    cv2.imwrite('debug_whole_body_hm_100_new.jpg', masked_image)
