...
class CropBodyLoss(torch.nn.Module):
  def __init__(self, opt):
    super(CropBodyLoss, self).__init__()
    self.crit = FocalLoss()
  ...
  def forward(self, outputs, batch):
    ...
    for s in range(opt.num_stacks):
      ...
      if opt.hm_hp and opt.hm_hp_weight > 0:                                    # 关节点的热力图
        ...
        # import pdb; pdb.set_trace()
        cv2.imwrite('0203_training_output_hm_hp_mse_21_mobilev3_10.jpg', hm_vis)
        ...
        hm_vis = np.repeat(hm_vis, 3, axis=-1)
        # import pdb; pdb.set_trace()
        cv2.imwrite('0203_training_label_hm_hp_mse_21_mobilev3_10.jpg', hm_vis)
...
class CropBodyTrainer(BaseTrainer):
  def __init__(self, opt, model, optimizer=None):
    super(CropBodyTrainer, self).__init__(opt, model, optimizer=optimizer)
